...
class Writeoff(models.Model):
    ...
    def create(self, vals):
        ...
        try:
            if vals['date_cancel'] is not None :
                date1 = vals['date_cancel']
        ...
        return super(Writeoff, self).create(vals)
    ...
    def write(self, vals):
        ...
        if vals.get('ethiopian_from') is not None:
            ...
            raise ValidationError(vals.get('ethiopian_from'))
        ...
        try:
            if vals['report_date'] is not None:
                
                date_str = vals['report_date']
                date_time_obj = date_str.split('-')
                Edate = EthiopianDateConverter.to_ethiopian(int(date_time_obj[0]),int(date_time_obj[1]),int(date_time_obj[2]))
                if type(Edate) ==   str:
                    vals['ethiopian_to'] = None
        ...
        return super(Writeoff, self).write(vals)
