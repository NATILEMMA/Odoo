...
#        perps = request.httprequest.form.getlist('perpertrators')
        ...
        if partner_id.is_league == False and partner_id.is_member == False and partner_id.is_leader == False:
            return request.render("member_dashboard.register_error_6", {'is_users': is_users})
        ...
        #     })
          ...
          return request.redirect('/my/complaint')
            ...
            return request.render("member_dashboard.complaint_end", {"is_users": is_users})
    ...
    def complaint_data(self, **kwargs):
        ...
        if not users:
            ...
            is_users.append("True")
        return request.render("member_dashboard.complaint_list", {'record': complaint,"users":users, 'is_users': is_users})
...
#            'perpertrators': perpertrators,
    ...
    def add_attachments(self, **kwargs):
        ...
        if partner.is_league == False and partner.is_member == False and partner.is_leader == False:
            return request.render("member_dashboard.register_error_7", {'is_users': is_users})
    ...
    def get_transfers(self, **kwargs):
        ...
        partner_id = request.env.user.partner_id
        users = request.env['res.partner'].search([('id','=',partner_id.id)])
        transfers = request.env['members.transfer'].sudo().search([('partner_id', '=', partner_id.id)])
        is_users = []
        if not users:
            is_users.append("False")
        else:
    ...
    def create_transfer(self, **kwargs):
        ...
        if kwargs and request.httprequest.method == 'POST':
            ...
            for field in set(['partner', 'partner_id', 'transfer_responsibility_leader', 'transfer_membership_org'
                              'transfer_responsibility_member', 'transfer_subcity_id', 'transfer_wereda_id',
            ...
            if values['partner'] and values['transfer']:
                transfers = request.env['members.transfer'].sudo().search([('id', '=', values['transfer'])])
                values.pop('partner')
                values.pop('transfer')
                ...
                if len(transfers) > 0:
                    return request.render("member_dashboard.register_error_5", {'is_users': is_users})

                if values['is_league'] == '' and values['is_member'] == '' and values['is_leader'] == '':
                    return request.render("member_dashboard.register_error_4", {'is_users': is_users})

                if values['is_league'] == 'True':
                    if values['transfer_as_a_league_or_member'] == 'member' and values['is_member'] == '' and values['is_leader'] == '':
                        return request.render("member_dashboard.register_error", {'is_users': is_users})
                    if values['transfer_as_a_league_or_member'] == '':
                        return request.render("member_dashboard.register_error_3", {'is_users': is_users})
                    values['league_org'] = partner.league_org
                    values['responsibility_in_org_league'] = partner .league_status
                ...
                if values['is_leader'] == 'True':
                    if values['transfer_as_a_leader_or_member'] == 'league' and values['is_league'] == '':
                        return request.render("member_dashboard.register_error_1", {'is_users': is_users})
                ...
                if values['is_member'] == 'True':
                    if values['transfer_as_a_league_or_member'] == 'league' and values['is_league'] == '':
                        return request.render("member_dashboard.register_error_2", {'is_users': is_users})
                    if values['transfer_as_a_league_or_member'] == '':
                        return request.render("member_dashboard.register_error_3", {'is_users': is_users})
                    values['membership_org'] = partner.membership_org.id
                ...
                return request.render("member_dashboard.register_end", {'is_users': is_users})
        is_leader = partner.is_leader
        is_member = partner.is_member
        is_league = partner.is_league
        fee = partner.membership_monthly_fee_cash + partner.membership_monthly_fee_cash_from_percent
    ...
    def profile(self, redirect=None, **post):
        ...
        if post and request.httprequest.method == 'POST':
            ...
            if partner.is_league == False and partner.is_member == False and partner.is_leader == False:
                return request.render("member_dashboard.register_error_8", {'is_users': is_users})
    ...
    def myreports(self,  **kw):
        ...
        if not users:
            ...
            is_users.append("True")
        return http.request.render('member_dashboard.my_report', {"users":users, 'is_users': is_users})
