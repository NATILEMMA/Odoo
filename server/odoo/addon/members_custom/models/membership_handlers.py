...
class AnnualPlansWereda(models.Model):
   ...
   def create(self, vals):
      ...
      quarter_1 = self.env['annual.plan.report'].sudo().create({
                     ...
                     'annual_plan_subcity_id': False,
                     'annual_plan_city_id': False,
...
class MembershipCityHandlers(models.Model):
   ...
   def revise_annual_planning(self):
      ...
      for record in self:
         plans = self.env['annual.plans'].search([('fiscal_year.state', '=', 'active')])
         for plan in plans:
            plan.state = 'draft'
...
class MembershipHandlersParent(models.Model):
   ...
   def revise_annual_planning(self):
      """This function will make city plans state draft"""
      for record in self:
         plans = self.env['annual.plans.subcity'].search([('fiscal_year.state', '=', 'active'), ('subcity_id', '=', record.id)])
         for plan in plans:
            plan.state = 'draft'
...
class MembershipHandlersChild(models.Model):
   ...
   def revise_annual_planning(self):
      """This function will make city plans state draft"""
      for record in self:
         plans = self.env['annual.plans.wereda'].search([('fiscal_year.state', '=', 'active'), ('wereda_id', '=', record.id)])
         for plan in plans:
            plan.state = 'draft'
