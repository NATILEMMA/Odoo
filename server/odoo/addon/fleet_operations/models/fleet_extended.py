...
class ProductProduct(models.Model):
    ...
    is_part = fields.Boolean(string='Is Spare Part?', default=False)
...
class FleetOperations(models.Model):
            ...
            if self.driver_id and self.vehicle_owner:
                ...
                if record.model_id and record.model_id.brand_id:
                    ...
                    if not record.license_plate:
                        lic_plate = ''
                    record.name = 'Emp/'+ str(record.vehicle_owner.name) +record.model_id.brand_id.name + '/' + \
                ...
                if record.model_id and record.model_id.brand_id:
                    ...
                    if not record.license_plate:
                        lic_plate = ''
                    record.name = 'Cus/'+ str(record.driver_id.name) + '/' +record.model_id.brand_id.name + '/' + \
                                  record.model_id.name + '/' + lic_plate
    ...
    driver_id = fields.Many2one('res.partner', 'Customer/Driver')
