...
class FleetVehicleLogServices(models.Model):
    ...
    def submit_request(self):
        print("self.vehicle_id.vechical_type_id.price", self.vehicle_id.vechical_type_id.price)
        if self.vehicle_id.vehicle_owner.login:
    ...
    def action_done(self):
      if self.additional_payment ==0 :
        ...
        if work_order.parts_ids:
            ...
            print("user", user)
            for loc in user:
                warehouse = self.env['stock.warehouse'].search([('operating_unit_id', '=', user.default_operating_unit_id.id)])
    ...
    checklist = fields.One2many('fleet.checklistevaluate', 'service_id',
...
class ServiceRepairLine(models.Model):
    ...
    complete = fields.Boolean(string='Completed', tracking=True, store=True)
    approve = fields.Many2one('res.users', string='Approved by')
    mechanic = fields.Many2one(related='repair_type_id.mechanic',required=True, readonly=False, store_true=True)
