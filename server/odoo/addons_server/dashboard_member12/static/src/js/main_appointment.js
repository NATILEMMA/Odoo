// Appointments
odoo.define('dashboard_member12.main_appointment', function (require) {
    'use strict';
    ...
    publicWidget.registry.OnlineAppointmentChanges = publicWidget.Widget.extend({

        selector: '#s2u_online_appointment',
        read_events: {
            'change select[name="city_id"]': '_subcityChange',
        },

         start: function () {
            var def = this._super.apply(this, arguments);
            this.$wereda = this.$('select[name="subcity"]');
         ...
         _weredaChange: function() {
             var $subcity = this.$('select[name="city_id"]');
