odoo.define('AADBO.product_new', function (require) {
    ...
    publicWidget.registry.registrationDetails = publicWidget.Widget.extend({
        ...
        read_events: {
            'change select[name="residential_subcity_id"]': '_subcityChange',
        },

         start: function () {
            var def = this._super.apply(this, arguments);
            this.$wereda = this.$('select[name="residential_wereda_id"]');
         ...
         _weredaChange: function() {
             var $subcity = this.$('select[name="residential_subcity_id"]');
    ...
    publicWidget.registry.OnlineAppointment = publicWidget.Widget.extend({
        ...
        _update_values: function () {
            var self = this;
            this._rpc({
                route: '/check_field_study',
                params: {
                    'input': $("#user_input").val()
                },
            }).then(function(result) {
                console.log(result.study)
