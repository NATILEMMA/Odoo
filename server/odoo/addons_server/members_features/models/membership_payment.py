...
class LeaguePayment(models.Model):
    ...
    def print_league_payslip(self):
        ...
        for record in self:
            return self.env.ref('create_league_payment_report').report_action(record._origin.id)

    def add_attachment(self):
        """this function will add attachments"""
        view_id = self.env.ref('members_handlers_payment.each_league_payment_form').id
...
class MembershipPayment(models.Model):
    ...
    def print_payslip(self):
        """This function will print payslips"""
        for record in self:
            return self.env.ref('create_member_payment_report').report_action(record._origin.id)

    def add_attachment(self):
        """this function will add attachments"""
        view_id = self.env.ref('members_handlers_payment.each_member_payment_form').id
        context = self._context.copy()
        return {
            'name':'Payment',
            'view_type':'form',
...
class DonationPayments(models.Model):
    ...
    def print_donor_payslip(self):
        """This function will print payslips"""
        for record in self:
            return self.env.ref('create_donor_payment_report').report_action(record._origin.id)
