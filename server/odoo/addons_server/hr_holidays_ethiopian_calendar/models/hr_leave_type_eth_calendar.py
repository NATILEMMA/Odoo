...
class HolidaysType(models.Model):
    ...
    ethiopian_from = fields.Date(string="From", store=True) # validity_start
    pagum_from = fields.Char(string="From", store=True)
    is_pagum_from = fields.Boolean(default='True', string="From")


    ethiopian_to = fields.Date(string="To", store=True) # validity_stop
    pagum_to = fields.Char(string="To", store=True)
    is_pagum_to = fields.Boolean(default='True', string="To")
    ...
    def create(self, vals):
        ...
        try:
            if vals['validity_start'] is not None:
                date1 = vals['validity_start']
            ...
            if vals['validity_stop'] is not None:
                date2 = vals['validity_stop']
        ...
        try:
            if res.validity_stop:
                date2 = res.validity_stop
                Edate2 = EthiopianDateConverter.to_ethiopian(date2.year,date2.month,date2.day)
                if type(Edate2) == date:
        ...
        try:
            if res.validity_start:
                date1 = res.validity_start
                Edate1 = EthiopianDateConverter.to_ethiopian(date1.year,date1.month,date1.day)
                if type(Edate1) == date:
    ...
    def write(self, vals):
        ...
        return super(HolidaysType, self).write(vals)
