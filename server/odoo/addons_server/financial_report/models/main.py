...
class Report_update(models.TransientModel):
    ...
    def _default_start_date(self):
        ...
        rec = self.env['reconciliation.time.fream'].search(
            [('date_from', '<=', today_date), ('date_to', '>', today_date)], limit=1)
        if rec:
            for r in rec:
                start_date = r.date_from
            return start_date
        else:
            fs_rec = self.env['fiscal.year'].search([('date_from', '<', today_date), ('date_to', '>=', today_date)],
                                                    limit=1)
            if fs_rec:
                for rr in fs_rec:
