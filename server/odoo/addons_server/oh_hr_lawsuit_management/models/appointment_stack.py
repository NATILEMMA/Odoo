...
class AppointmentStack(models.Model):
    ...
    court_name = fields.Char(required=True, string='Court Name',translate=True)
    judge = fields.Char(required=True, string='Judge', help='Name of the Judge',translate=True)
    lawyer = fields.Many2one('res.partner', string='Lawyer', help='Choose the contact of Layer from the contact list')
    ref_no = fields.Char(string="Reference Number", required=True,translate=True)
