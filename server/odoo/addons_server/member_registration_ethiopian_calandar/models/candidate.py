...
class CandidateMembers(models.Model):
    ...
    ethiopian_to = fields.Date(string="Becomes A Member On")
    pagum_to = fields.Char(string="Becomes A Member On")
    is_pagum_to = fields.Boolean(default='True', string="Becomes A Member On")
    ...
    def create(self, vals):
        ...
        try:
            ...
            pass
        if res.age == 0:
            raise UserError(_("Please Add The Appropriate Age Of The Candidate"))
        if res.age < 15:
    ...
    def write(self, vals):
        ...
        try:
            ...
            if vals['ethiopian_to'] is not None:
                ...
                date_gr = EthiopianDateConverter.to_gregorian(int(date_time_obj[0]), int(date_time_obj[1]),
                                                              int(date_time_obj[2]))
                Edate1 = EthiopianDateConverter.to_ethiopian(date_gr.year, date_gr.month, date_gr.day)
                vals['becomes_member_on'] = date_gr
                if type(Edate1) == str:
                    vals['ethiopian_to'] = None
        ...
        #     if not vals.get('becomes_member_on'):
        #         raise UserError(_("Please Add The Date When The Candidate Becomes a Member"))
        #     else:
        #         if type(vals.get('becomes_member_on')) == str:
        #             date_new = datetime.strptime(vals.get('becomes_member_on'), '%Y-%m-%d').date()
        #             if date_new > date.today():
        #                 raise UserError(_("You Have To Pick A Date Before Today."))
        #         if type(vals.get('becomes_member_on')) == date:
        #             date_new = vals.get('becomes_member_on')
        #             if date_new > date.today():
        #                 raise UserError(_("You Have To Pick A Date Before Today."))
            ...
            for archive in all_archived:
                if archive.archived:
                    ...
                    if type(Edate1) == date:
                        ...
                        pass
        return super(CandidateMembers, self).un_archive_record()
