...
class AccountBudgetPost(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', required=True,
                                 default=lambda self: self.env['res.company']._company_default_get(
                                     'account.budget.post'))
    is_transfer_code = fields.Boolean()
...
class BudgetLines(models.Model):
    ...
    def _compute_practical_amount(self):
        for line in self:
            ...
            bud_code = line.general_budget_id.id
            state = "posted"

            _logger.info("acc_ids:%s",acc_ids)
            _logger.info("bud_code:%s",bud_code)
            _logger.info("aaaaaaaaaaa:%s",line.analytic_account_id.id,)
            ...
            if line.analytic_account_id.id:
                self.env.cr.execute("""
                    ...
                    WHERE analytic_account_id=%s
                        ...
                        AND account_id=ANY(%s) AND budget_line_code=%s AND parent_state=%s """,
                                    (line.analytic_account_id.id, date_from, date_to, acc_ids,bud_code,state))
                result = self.env.cr.fetchone()[0] or 0.0


            _logger.info("YYYYYYYYYYYYYYYY %s",result)
