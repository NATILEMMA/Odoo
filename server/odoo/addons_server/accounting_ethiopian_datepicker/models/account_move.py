...
class AccountMove(models.Model):
    ...
    pagum_three = fields.Char(string="in ethiopian date",translate=True)
    ...
    def create(self, vals):
        ...
        try:
            ...
            if vals['ethiopian_to'] is not None:
                ...
                date_gr_three = EthiopianDateConverter.to_gregorian(int(date_time_obj[0]), int(date_time_obj[1]),
                                                                   int(date_time_obj[2]))
                Edate1 = EthiopianDateConverter.to_ethiopian(date_gr_three.year, date_gr_three.month, date_gr_three.day)

                vals['invoice_date_due'] = date_gr_three
    ...
    def initial_date(self, data):
        ...
        if len(id[0]) <= 0:
            ...
            if search.ethiopian_three != False and search.pagum_three == False:
                From.append(search.ethiopian_three)
            if search.ethiopian_three == False and search.pagum_three != False:
                date_three_str = str(search.pagum_three).split('/')
                date_three = date_three_str[2] + '-' + date_three_str[0] + '-' + date_three_str[1]
                From.append(date_three)
            if search.ethiopian_three == False and search.pagum_three == False:
                today = datetime.now()
    ...
    def action_in_progress(self):
        date_start = datetime.strptime(str(self.invoice_date).split(' ')[0], '%Y-%m-%d').date()
        if self.invoice_date_due < date_start:
                    raise ValidationError(_("you stat date should be less than end date"))
        return super(AccountMove, self).action_in_progress()
