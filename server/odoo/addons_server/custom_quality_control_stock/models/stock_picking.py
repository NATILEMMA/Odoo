...
class ProductCategory(models.Model):
    ...
    cat_code = fields.Char(string="Code", translate=True)
...
class StockMove(models.Model):
    ...
    inspection_success_qty = fields.Float()
    inspection_failed_qty = fields.Float()
    is_insepction_needed = fields.Boolean(default=False)
    ...
    def _generate_serial_numbers(self, next_serial_count=False):
        ...
        move_lines_commands = self._generate_serial_move_line_commands(lot_new)
    ...
    def action_show_details(self,next_serial_count=False):
        ...
        if not last_id:
            ...
            if int(get_next_serial) > 0:
                _logger.info("*******lot_names**** %s",lot_new)
                _logger.info("*******lot_new**** %s",lot_new[1])        
                self.next_serial = lot_new[0]
                ...
                if self.is_insepction_needed is False:
                    ...
                    raise exceptions.UserError(
                                _(
                                    "To assign a serial number, an inspection result is Inspection success and failed quantity required...! To Force assign uncheck the Inspection Required box."
    ...
    def button_validate(self):
        if self.is_insepction_needed is True:
            ...
            if self.is_assigned is False:  
                ...
                raise ValidationError(_("Done Inspection Zero, you must complete the inspection.") ) #\n\n Or Or, uncheck Inspection needed box")
                ...
                raise exceptions.UserError(
                        _(
                            ...
                            " There are still %s inspections left to complete the total inspection.") %str(-(int(self.done_inspections)-( self.created_inspections))
                        )
                    )
            else:
