...
class Donors(models.Model):
    ...
    def create(self, vals):
        ...
        for i in range(0, len(pick1)):
            ...
            if i == (len(pick1) - 1):
                ...
                if today.month < date1.month:
                    vals['age'] = (today.year - date1.year) - 1
                else:
                    if today.month == date1.month and today.day < date1.day:
                        vals['age'] = (today.year - date1.year) - 1
                    else:
                        vals['age'] = today.year - date1.year
        ...
        return super(Donors, self).create(vals)
    ...
    def write(self, vals):
        ...
        try:
            ...
            if vals['date'] is not None:
                ...
                if date_str >= today:
                    raise UserError(_("You Have To Pick A Date Before Today."))
                if today.month < date_str.month:
                    vals['age'] = (today.year - date_str.year) - 1
                else:
                    if today.month == date_str.month and today.day < date_str.day:
                        vals['age'] = (today.year - date_str.year) - 1
                    else:
                        vals['age'] = today.year - date_str.year


        except:
            pass

        return super(Donors, self).write(vals)
