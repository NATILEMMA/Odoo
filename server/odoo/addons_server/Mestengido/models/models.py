...
class Mestengido(models.Model):
    ...
    requester_id = fields.Many2one('hr.employee', string='Requester', default=_default_employee_id, domain=lambda self: self._get_employee_id_domain(),  tracking=True)
    ...
    requster_signed_by = fields.Many2one('hr.employee','Signed By', help='Name of the person that signed the SO.', copy=False)
    ...
    def action_approve(self):
        self.state = "approved"
        super(Mestengido, self).write({'state':'approved'})
    ...
    def action_reset(self):
        self.state = "draft"
        super(Mestengido, self).write({'state':'draft'})
...
class PurchaseOrder(models.Model):
    ...
    mestengido_id = fields.Many2one('mest.order')
...
class MestengidoOrder(models.Model):
    ...
    order_line_details = fields.One2many('mest.order.lines.details', 'order_id', string='Order Lines', default={})
    ...
    def get_mestengido(self):
            ...
            return {
                ...
                'domain': [('id', '=', self.purchas_order.id)],
                'context': "{'create': False}"
            }
    
    def compute_count(self):
        for record in self:
            record.mestengido_count = self.env['mest.order'].search_count(
                [('id', '=', self.id)])
    ...
    def _compute_monthly_request(self):
        # try:
        month = self.month
        search_request = self.env['mest.mest'].sudo().search([('month','=',month),('state','in',['approved'])])
        requests = []
        ...
        if self.is_ture == True:
            ...
            for request in self.requests:
                ...
                for line in request.order_line:
                    ...
                    data1['order_id'] = line.order_id.id
                    data1['product_id'] = line.product_id.id
                    data1['name'] = line.name
                    data1['product_uom'] = line.product_uom.id
                    data1['product_qty'] = line.product_qty
                    data1['approved_qty'] = line.product_qty
                    data1['department_id'] = request.department_id

                    
                    order.append((0,0,data))
                    requester_order.append((0,0,data1))


                        
            _logger.info("Data %s",order)
            self['order_line'] = order
            self['order_line_details'] = requester_order
    ...
    def action_change_to_purchase_order(self):
        ...
        if post_request.total_price <= amount_valid.amount:
            ...
            data['partner_id'] = users.partner_id.id
            data['user_id'] = self.env.uid
            data['date_planned'] = today
            data['mestengido_id'] = self.id
            # data['state'] = 'dreft'
            ...
            self.purchas_order = create_PO.id
        else:
...
class MestengidoOrderLinesDetails(models.Model):
    _name = 'mest.order.lines.details'
    _description = 'Mestengido Order Lines'
    _order = 'order_id, sequence, id'
    ...
    product_qty = fields.Integer(string='Quantity', digits='Product Unit of Measure',force_save="1")
    product_uom_qty = fields.Float(string='Total Quantity', compute='_compute_product_uom_qty')
    approved_qty = fields.Integer(string='Purchased Qty', digits='Product Unit of Measure',force_save="1")
    date_planned = fields.Datetime(string='Scheduled Date', index=True)
    taxes_id = fields.Many2many('account.tax', string='Taxes', domain=['|', ('active', '=', False), ('active', '=', True)])
    product_uom = fields.Many2one('uom.uom', string='Unit of Measure', domain="[('category_id', '=', product_uom_category_id)]")
    product_uom_category_id = fields.Many2one(related='product_id.uom_id.category_id')
    ...
    @api.model
    def create(self, values):
        line = super(MestengidoOrderLinesDetails, self).create(values)
        if self.env.context.get('keep_line_sequence'):
            line.order_id._reset_sequence()
        return line
