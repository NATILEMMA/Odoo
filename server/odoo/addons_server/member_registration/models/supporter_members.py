...
class SupportingMembers(models.Model):
    ...
    def postpone_approval(self):
        ...
        for record in self:
            ...
            mail_temp = self.env.ref('supporter_postpone')
    ...
    def deny_candidancy(self):
        ...
        for record in self:
            ...
            mail_temp = self.env.ref('supporter_denial')
            mail_temp.send_mail(record.id)
            self.deactivate_activity(record)
    ...
    def back_to_new(self):
        ...
        for record in self:
            ...
            if record.attachment_amount < record.click_counter:
                ...
                mail_temp = self.env.ref('supporter_revival')
                mail_temp.send_mail(record.id)
                self.deactivate_activity(record)
    ...
    def create_candidate(self):
        ...
        for record in self:
            ...
            mail_temp = self.env.ref('supporter_approval')
            mail_temp.send_mail(record.id)
            self.deactivate_activity(record)
            user = self.env.user
