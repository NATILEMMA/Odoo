...
class SendSupporters(models.TransientModel):
    ...
    def action_done(self):
        ...
        if self.main_office and self.cells:
            ...
            mail_temp = self.env.ref('members_custom.supporter_acceptance')
            ...
            # message = _("%s's 3 month has Passed. Please Make A Decision About The State of Their Candidacy.") % (str(wizard.supporter_id.name))
            ...
            #     })
            #     self.main_office.main_admin.notify_warning(message, title, True)
            ...
            #         'activity_type_id': activity_type.id                
            #     })
            #     self.cells.cell_admin.notify_warning(message, title, True)

            # wizard.supporter_id.deactivate_activity(wizard.supporter_id)
        # if wizard.candidate_id:
        #     if self.main_office and self.cells:
        #         wizard.candidate_id.write({
        #             'main_office_id': self.main_office.id,
        #             'cell_id': self.cells.id
        #         })
        #     else:
        #         raise UserError(_("Please Fill In The Given Informations"))
            # message = _("%s's 6 month has Passed. Please Make A Decision About The State of Their Membership.") % (str(wizard.candidate_id.name))
