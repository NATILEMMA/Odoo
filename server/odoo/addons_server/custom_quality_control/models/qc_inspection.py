...
class QcInspection(models.Model):
    ...
    @api.onchange('passed_total')
    def check_qauntity_validation(self):
        _logger.info("ffffffffffffffffffff parent")
        for record in self:
            _logger.info("qty-----validation %s",record.ids)

            _logger.info("qty-----validation %s",record.ids)
            _logger.info("qty-----validation", record.inspection_id)
            _logger.info("qty-----validation", record.passed_total)
    ...
    def action_confirm(self):
        for inspection in self:
            ...
            for line in inspection.inspection_lines:
                ...
                passed.append(line.passed_total)
            largest_num = max(passed)
            smallest_num = min(passed)
            _logger.info("............passed largest_num:%s",largest_num)
            _logger.info("............passed smallest_num:%s",smallest_num)

            _logger.info("............passed smallest_num:%s",self.picking_id)
            _logger.info("............passed smallest_num:%s",self.picking_id)

            transfer = self.picking_id

            if inspection.success:
                for line in transfer.move_ids_without_package:
                    _logger.info("@@@@@@@@@@@@@@@@@@@@@@@ %s",self.product_id)
                    _logger.info("@@@@@@@@@@@@@@@@@@@@@@@ %s",line.product_id)

                    if line.product_id.id == self.product_id.id:
                        line.inspection_success_qty = smallest_num
                        line.inspection_failed_qty = self.qty-smallest_num
...
class QcInspectionLine(models.Model):
    ...
    passed_total = fields.Float(string="Passed", default=0.0)
    value_setted = fields.Boolean()


    @api.onchange('quantitative_value','qualitative_value')
    def check_success(self):
        for record in self:
            if record.success == True:
                record.value_setted = True
            else:
                record.value_setted = True
    ...
    def check_qauntity_validation(self):
        for record in self:
            ...
            parent_id = record.inspection_id.ids
            search = self.env['qc.inspection'].search([('id','in',parent_id)], limit=1)
