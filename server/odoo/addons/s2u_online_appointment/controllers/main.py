...
class OnlineAppointment(http.Controller):
    ...
    def register(self, **post):
        ...
        if post and request.httprequest.method == 'POST':
            ...
            subcity = request.env['membership.handlers.parent'].search([('id', '=', values['residential_subcity_id'])])
            if subcity:
                values['residential_subcity'] = subcity.name
            woreda = request.env['membership.handlers.branch'].search([('id', '=', values['residential_wereda_id'])])
            if woreda:
                values['residential_wereda'] = woreda.name
