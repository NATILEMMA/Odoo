...
class AccountAssetRemove(models.TransientModel):
    ...
    estimated_value = fields.Float(String = "Estimated Price", default=lambda self: self._default_estimated_value())
    
    remove_method = fields.Selection([('scrap','Scrap'),('sale','Sale')], string='Remove Method', required=True, default='sale',help="Choose the method to use to remove the asset \n" )
    ...
    def _get_default_location_id(self):
        ...
        stock_quantity = self.env['stock.quant'].search([('product_id','=',product.id),('on_hand','=',True),('quantity','>',0)])
        location_lists = []
        for stock in stock_quantity:
            if not stock.location_id.id in location_lists:
                
                location_lists.append(stock.location_id.id)
        if location_lists:
    ...
    partner_id = fields.Many2one(
        'res.partner', 'Delivery Partner')
    location_dest_id = fields.Many2one(
        'stock.location', 'Destination Location')
    

    @api.onchange('remove_method')
    def change_sale_value(self):
        ...
        if self.remove_method == 'scrap':
            self.sale_value = 0.0
            if location_lists:
                return {'domain': {'location_id': [('id','in', location_lists)],'scrap_location_id': [('scrap_location', '=', True)]},'default': {'location_id': location_lists[0]}}
            else:
                return {'domain': {'location_id': [('id','in', location_lists)]}}
        else :
             return {'domain': {'location_id': [('id','in', location_lists)]},'default': {'location_id': location_lists[0]}}
    ...
    @api.model
    def _default_estimated_value(self):
        return self._get_sale()["estimated_value"]
    ...
    def _get_sale(self):
        ...
        sale_value = 0.0
        estimated_value = 0
        if asset.estimated_value > 0.0:
            estimated_value = asset.estimated_value
        else:
            estimated_value = asset.value_residual
        ...
        for line in inv_lines:
            ...
            if (
                ...
                sale_value += amount
        return {"sale_value": sale_value, "account_sale_id": account_sale_id ,"estimated_value":estimated_value}
    ...
    def remove(self):
        ...
        stock_picking_type = self.env['stock.picking.type'].search([('code','=','outgoing')])[0]
        if not ((self.partner_id.property_stock_customer.id or stock_picking_type.default_location_dest_id.id or self.remove_method == 'scrap' )):
            
            raise UserError(
                _(
                    "Default destination location not given! Please set default location in Inventory >> Configuration >> Operation Types >> Delivery Orders >> field (Default destination location)"
                )
            )
        ...
        if self.remove_method == 'scrap':
            # try:
                ...
                if asset.tracking != 'none':
                    vals.update({'lot_id': asset.lot_id.id,})
                    

                asset.scrap_link = scrap.sudo().create(vals).id
        ...
        asset.write({"state": "removed", "date_remove": self.date_remove,"sold_price" : self.sale_value,"estimated_value":self.estimated_value})
        ...
        product_move_values = [
        {'name':str(fields.Datetime.today),'product_id':product.id,'product_uom_qty':1.0,'product_uom':product.uom_id.id}
        ]

        if self.remove_method == 'sale':
            ...
            vals = {
                ...
                'location_id': self.location_id.id,
                'company_id': self.company_id.id,
                'move_ids_without_package': [(0, 0, val) for val in product_move_values],
                'location_dest_id':self.partner_id.property_stock_customer.id or stock_picking_type.default_location_dest_id.id
                }
            stock_picking_id = stock_picking.sudo().create(vals).id
            if  stock_picking_id:
             print(stock_picking_id)
             asset.delivery_transfer_link = stock_picking_id
