...
class AccountAsset(models.Model):
    ...
    use_leap_years = fields.Boolean(
        ...
        "depreciation base / number zof years.\n "
    ...
    sold_price = fields.Float(String = "Sold price",default = 0,copy=False)
    estimated_value = fields.Float(String = "Estimated price",readonly=False, default=lambda self: self._default_estimated_value(),copy=False)
    ...
    product_id = fields.Many2one(
        ...
        required=True, states={'done': [('readonly', True)]}, check_company=True,copy=False)
    
    
    tracking = fields.Selection('Product Tracking', readonly=True, related="product_id.tracking")
    lot_id = fields.Many2one(
        'stock.production.lot', 'Lot/Serial',
        states={'done': [('readonly', True)],'open': [('readonly', True)],'close': [('readonly', True)]} ,copy=False, domain="[('product_qty','>=',0.0),('product_id', '=', product_id), ('company_id', '=', company_id),('associated_asset', '=', False)]")
    scrap_link = fields.Many2one("stock.scrap", string = "Scrap link" ,copy=False)
    delivery_transfer_link = fields.Many2one("stock.move" ,String = "Sale Delivery link" ,copy=False)
    ...
    def onchange_product_id(self):
            for record in self:
                ...
                return {'domain': {'lot_id': [('product_qty','>=',0.0),('product_id', '=', product_id), ('company_id', '=', company_id),('associated_asset', '=', False)]}}

    @api.onchange('lot_id')
    def _print_the_log(self):
        print(self.lot_id.product_qty)
    ...
    def write(self, vals):
        ...
        for asset in self:
            if asset.lot_id:
                lot = self.env['stock.production.lot'].search([('id', '=' ,asset.lot_id.id)])
                if lot.associated_asset == False:
                    lot.associated_asset = asset.id
    ...
    def validate(self):
        for asset in self:
            if asset.tracking and asset.tracking != 'none':
                if not asset.lot_id :
                    raise UserError(
                            _(
                                "The product is tracked using serial/lot"
                                "Please fill the lot field by setting the form state to draft !!!"
                            )
                        )
            if asset.lot_id:
                lot = self.env['stock.production.lot'].search([('id', '=' ,asset.lot_id.id)])
                lot.associated_asset = asset.id
                print("lot *",lot,asset,lot.associated_asset)
    ...
    def remove(self):
        ...
        for rec in self:
            if rec.tracking and rec.tracking != 'none':
                if not rec.lot_id :
                    raise UserError(
                            _(
                                "The product is tracked using serial/lot"
...
class ProductTemplate(models.Model):
    ...
    asset_category_custom_id = fields.Many2one('account.asset.profile', string='Asset Type', company_dependent=True, ondelete="restrict")
