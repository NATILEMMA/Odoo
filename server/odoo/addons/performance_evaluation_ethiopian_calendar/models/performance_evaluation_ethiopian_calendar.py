...
class PerformanceEvaluationProgram(models.Model):
    ...
    ethiopian_from = fields.Date(string="Date From", store=True) # date_from
    pagum_from = fields.Char(string="Date From", store=True)
    is_pagum_from = fields.Boolean(default='True', string="Date From")


    ethiopian_to = fields.Date(string="Date To") # date_to
    pagum_to = fields.Char(string="Date To")
    is_pagum_to = fields.Boolean(default='True', string="Date To")

    ethiopian_three = fields.Date(string="Evaluation Date") # date_of_evaluation
    pagum_three = fields.Char(string="Evaluation Date")
    is_pagum_three = fields.Boolean(default='True', string="Evaluation Date")
    ...
    def create(self, vals):
        ...
        try:
            ...
            if vals['date_of_evaluation'] is not None:
                date2 = vals['date_of_evaluation']
                date_time_obj2 = date2.split('-')
        ...
        return super(PerformanceEvaluationProgram, self).create(vals)
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            try:
                if vals['date_of_evaluation'] is not None:
                    date_str = vals['date_of_evaluation']
                    date_time_obj = date_str.split('-')
                    Edate = EthiopianDateConverter.to_ethiopian(int(date_time_obj[0]),int(date_time_obj[1]),int(date_time_obj[2]))
                    if type(Edate) ==  str:
                        vals['ethiopian_three'] = None
            ...
            try:
                if vals['ethiopian_three'] is not None:
                    ...
                    date_gr = EthiopianDateConverter.to_gregorian(int(date_time_obj[0]), int(date_time_obj[1]),
                                                                int(date_time_obj[2]))
                    Edate1 = EthiopianDateConverter.to_ethiopian(date_gr.year, date_gr.month, date_gr.day)
                    vals['date_of_evaluation'] = date_gr
                    if type(Edate1) == str:
                        vals['ethiopian_three'] = None
                        vals['pagum_three'] = Edate1
                        vals['is_pagum_three'] = False
        ...
        return super(PerformanceEvaluationProgram, self).write(vals)


    def send_evaluation(self):
        res = super(PerformanceEvaluationProgram, self).send_evaluation()
        date1 = self.date_of_evaluation
        Edate1 = EthiopianDateConverter.to_ethiopian(date1.year,date1.month,date1.day)
        if type(Edate1) == date:
    ...
    def _find_period_of_evaluation(self):
        ...
        for record in self:
            if record.year:
                ...
                date1 = record.date_from
                Edate1 = EthiopianDateConverter.to_ethiopian(date1.year,date1.month,date1.day)
                if type(Edate1) == date:
                    record.ethiopian_from = Edate1
                if type(Edate1) == str:
                    record.pagum_from =  Edate1
                    record.is_pagum_from = False
                record.date_to = record.year.date_to
                date2 = record.date_to
                Edate2 = EthiopianDateConverter.to_ethiopian(date2.year,date2.month,date2.day)
                if type(Edate2) == date:
                    record.ethiopian_to = Edate2
                if type(Edate2) == str:
                    record.pagum_to =  Edate2
                    record.is_pagum_to = False
