...
class HrLegalEmployeeMaster(models.Model):
    ...
    employee_legal_count = fields.Integer(String="legal_count", default = 0,help="this is a counter field for graph view. since computed fields are not possible on graph view we use this instead")
    def _legal_count(self):
        for each in self:
            legal_ids = self.env['hr.lawsuit'].search([('employee_id', '=', each.id)])
            each.legal_count = len(legal_ids)
            each.employee_legal_count = len(legal_ids)
