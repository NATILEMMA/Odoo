...
class CandidateMembers(models.Model):
    ...
    def postpone_approval(self):
        ...
        for record in self:
            ...
            mail_temp = self.env.ref('member_registration.candidate_postpone')
    ...
    def deny_membership(self):
        ...
        for record in self:
            ...
            mail_temp = self.env.ref('member_registration.candidate_denial')
            mail_temp.send_mail(record.id)
            self.deactivate_activity(record)
    ...
    def back_to_new(self):
        ...
        for record in self:
            ...
            #     record.click_counter = record.attachment_amount + 1
                ...
                mail_temp = self.env.ref('member_registration.candidate_revival')
                mail_temp.send_mail(record.id)
                self.deactivate_activity(record)
