...
class Grade(models.Model):
  ...
  _sql_constraints = [
                      ('check_on_fixed_wage', 'CHECK(minimum_wage <= fixed_wage AND maximum_wage >= fixed_wage)', 'Minimum wage must less or equal to fixed Wage and Maximum wage must less or equal to fixed Wage'),
                      ('check_on_minimum_wage', 'CHECK(minimum_wage > 0 AND minimum_wage < fixed_wage AND minimum_wage <= maximum_wage)', 'Minimum wage must be less than Maximum Wage and greater than 0')
                     ]
  ...
  #     if (not(vals.get('maximum_wage') > 0 and vals.get('maximum_wage') >= vals.get('maximum_wage'))):
  #         raise UserError (("Maximum wage must be more than Minimum Wage and 0"))
  #     if(not (vals.get('maximum_wage') > 0 and vals.get('maximum_wage') <= vals.get('maximum_wage'))):
  #         raise UserError (("Minimum wage must be less or equal than  Maximum Wage and 0"))
  #     if (vals.get('maximum_wage') >= vals.get('maximum_wage') and vals.get('maximum_wage') <= vals.get('maximum_wage')):
  #         raise UserError (("Fixed wage must be less or equal to  Maximum Wage and  must be greater or equal to Minimum wage"))
  #     return super(Grade, self).write(vals)
  
  def create(self,vals):
      print(vals)
      if (not(vals[0]['maximum_wage'] > 0 and vals[0]['maximum_wage'] >= vals[0]['minimum_wage'])):
          raise UserError (("Maximum wage must be more than Minimum Wage and 0"))
      elif (not (vals[0]['minimum_wage'] > 0 and vals[0]['minimum_wage'] <= vals[0]['maximum_wage'])):
          raise UserError (("Minimum wage must be less or equal than  Maximum Wage and 0"))
      elif (not (vals[0]['fixed_wage'] >= vals[0]['minimum_wage'] and vals[0]['fixed_wage'] <= vals[0]['maximum_wage'])):
          raise UserError (("Fixed wage must be less or equal to Maximum Wage and Fixed wage must be greater or equal to Minimum wage"))
      return super(Grade, self).create(vals)
