...
class Complaints(models.Model):
  ...
  def send_to_subcity_complaint_handler(self):
    ...
    for complaint in complaints:
        complaint.write({
          'subcity_id': complaint.wereda_id.parent_id,
          'subcity_handler': complaint.subcity_id.complaint_handler,
          'state': 'transferred',
          'transfer_1': True,
          'date_of_remedy_subcity': datetime.now() +  timedelta(days=complaint.duration_of_remedy_subcity)
        })
        model = self.env['ir.model'].search([('model', '=', 'member.complaint'), ('is_mail_activity', '=', True)])
        activity_type = self.env['mail.activity.type'].search([('name', '=', 'Complaint')], limit=1)
        activity = self.env['mail.activity'].sudo().create({
        ...
        mail_temp = self.env.ref('members_features.complaint_transfered_to_subcity')
  ...
  def send_to_city_complaint_handler(self):
    ...
    for complaint in complaints:
        complaint.write({
          'city_id': complaint.wereda_id.parent_id.city_id,
          'city_handler': complaint.city_id.complaint_handler,
          'state': 'transferred to city',
          'transfer_2': True,
          'date_of_remedy_city': datetime.now() +  timedelta(days=complaint.duration_of_remedy_city)
        })
        model = self.env['ir.model'].search([('model', '=', 'member.complaint'), ('is_mail_activity', '=', True)])
        activity_type = self.env['mail.activity.type'].search([('name', '=', 'Complaint')], limit=1)
        activity = self.env['mail.activity'].sudo().create({
        ...
        mail_temp = self.env.ref('members_features.complaint_transfered_to_city')
        mail_temp.send_mail(complaint.id)
  ...
  def transfer_button(self):
    ...
    for record in self:
        record.write({
          'subcity_id': record.wereda_id.parent_id,
          'subcity_handler': record.subcity_id.complaint_handler,
          'state': 'transferred',
          'transfer_1': True,
          'date_of_remedy_subcity': datetime.now() +  timedelta(days=record.duration_of_remedy_subcity)
        })
        model = self.env['ir.model'].search([('model', '=', 'member.complaint'), ('is_mail_activity', '=', True)])
        activity_type = self.env['mail.activity.type'].search([('name', '=', 'Complaint')], limit=1)
        activity = self.env['mail.activity'].sudo().create({
  ...
  def transfer_city_button(self):
    ...
    for record in self:
        record.write({
          'city_id': record.subcity_id.city_id,
          'city_handler': record.city_id.complaint_handler,
          'state': 'transferred to city',
          'transfer_2': True,
          'date_of_remedy_city': datetime.now() +  timedelta(days=record.duration_of_remedy_city)
        })
        model = self.env['ir.model'].search([('model', '=', 'member.complaint'), ('is_mail_activity', '=', True)])
        activity_type = self.env['mail.activity.type'].search([('name', '=', 'Complaint')], limit=1)
        activity = self.env['mail.activity'].sudo().create({
