...
class BudgetPlanning(models.Model):
    ...
    def action_set_to_draft(self):
        ...
        for budget_line in self.work_stream_line:
            ...
            analytic_account.write({
                'is_allowed': False,
                'budget_analytic_account': False
            })
            for v in budget.teams:
                members = self.env['hr.employee'].search([('id','=',v.id)])
                Is_allowed_accounts = []
                # for index,a in enumerate(members.accountAnalytic):
                #     if a.id == analytic_account.id:
                #         pass
                #     else:
                #         Is_allowed_accounts.append(a.id)

                # _logger.info("MMMMMMMMMMMMMMMMMMMMMMMMMM %s",Is_allowed_accounts)
                ...
                # members.write({
                #     "is_allowed": False,
                #     'accountAnalytic': [(6,0,Is_allowed_accounts)] 
                # })
    ...
    def action_ceo_approve(self):
        ...
        for budget_line in self.work_stream_line:
            ...
            analytic_account.write({
                'is_allowed': True,
                'budget_analytic_account': True
            })
            for v in budget.teams:
                members = self.env['hr.employee'].search([('id','=',v.id)])
                _logger.info("MMMMMMMMMMMMMMMMMMMMMMMMMM %s",members.name)
                Is_allowed_accounts = []
                # for index,a in enumerate(members.accountAnalytic):
                #     if index == 0:
                #         Is_allowed_accounts.append(a.id)
                #         Is_allowed_accounts.append(analytic_account.id)
                #     else:
                #         Is_allowed_accounts.append(a.id)

                # _logger.info("MMMMMMMMMMMMMMMMMMMMMMMMMM %s",Is_allowed_accounts)

                # if len(Is_allowed_accounts) > 0:
                #     Is_allowed_accounts = Is_allowed_accounts
                # else:
                #     Is_allowed_accounts.append(analytic_account.id)
                
                # _logger.info("MMMMMMMMMBBBBBBBBBBBBBBBBB %s",Is_allowed_accounts)
                # filtered_analytic_account = res = [*set(Is_allowed_accounts)]
                # _logger.info("MMMMMMMMMBBBBBBBBBBBBBBBBB %s",filtered_analytic_account)
                ...
                if not search_mapping:
                    create = search_mapping.create({
                        'employee_id': members.id,
                    ...
                    if search_mapping.is_allowed == True:
                        pass
                    else:
                        search_mapping.write({
                            'is_allowed': True
                        })
                ...
                # members.write({
                #     "is_allowed": True,
                #     'accountAnalytic': [(6,0,filtered_analytic_account)] 
                # })
