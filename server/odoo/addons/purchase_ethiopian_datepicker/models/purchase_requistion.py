...
class purchaseRequisition(models.Model):
    ...
    def action_in_progress(self):
        if not self.date_to or not self.ordering_date:
            raise ValidationError(_(
                'please select start date and end date for the tender'))
        if self.date_to > self.ordering_date:
            raise ValidationError(_(
    ...
    def create(self, vals):
        ...
        try:
            ...
            if vals['ethiopian_to'] is not None:
                ...
                vals['ordering_date'] = date_gr_from
        ...
        try:
            if vals['date_end'] is not None:
                ...
                if type(Edate1) == date:
                    ...
                    pass
            if vals['ordering_date'] is not None:
                date1 = vals['ordering_date']
                date_time_obj = str(date1).split(' ')
                date_time_obj = date_time_obj[0].split('-')
                print('date_time_obj', date_time_obj)
                Edate1 = EthiopianDateConverter.to_ethiopian(int(date_time_obj[0]), int(date_time_obj[1]),
    ...
    def action_in_progress(self):
        date_start = datetime.strptime(str(self.date_end).split(' ')[0], '%Y-%m-%d').date()
        if self.ordering_date < date_start:
