...
class DisciplinaryAction(models.Model):
    ...
    ethiopian_from = fields.Date(string="Joined Date", store=True) # joined_date
    pagum_from = fields.Char(string="Joined Date", store=True)
    is_pagum_from = fields.Boolean(default='True', string="Joined Date")
    ...
    def create(self, vals):
        ...
        try:
            if vals['joined_date'] is not None:
                date1 = vals['joined_date']
        ...
        res = super(DisciplinaryAction, self).create(vals)

        try:
            if res.joined_date:
                date1 = res.joined_date
                Edate1 = EthiopianDateConverter.to_ethiopian(date1.year,date1.month,date1.day)
                if type(Edate1) == date:
    ...
    def write(self, vals):
        ...
        try:
            ...
            pass
        return super(DisciplinaryAction, self).write(vals)
