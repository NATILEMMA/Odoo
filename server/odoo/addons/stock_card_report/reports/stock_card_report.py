...
class StockCardView(models.TransientModel):
    ...
    price_total = fields.Float(compute = "_price_total_calculator")
    ...
    def _price_total_calculator(self):
        for record in self:
            if record.price_unit and record.product_qty:
...
class StockCardReport(models.TransientModel):
    ...
    def _compute_results(self):
        ...
        self._cr.execute(
            """
            SELECT move.reference, move.date, move.product_id,move.price_unit, move.product_qty,
                move.product_uom_qty, move.product_uom,
                move.location_id, move.location_dest_id,move.cost_price,
        ...
        stock_card_result = self._cr.dictfetchall()

        print("**************************************",stock_card_result)
        ...
        self.results = [ReportLine.new(line).id for line in stock_card_result]
        print("here")
        print(self.results)
