...
class TestQualityControl(TransactionCase):
    def setUp(self):
        ...
        self.test = self.env.ref("custom_quality_control.qc_test_1")
        self.val_ok = self.env.ref("custom_quality_control.qc_test_question_value_1")
        self.val_ko = self.env.ref("custom_quality_control.qc_test_question_value_2")
        self.qn_question = self.env.ref("custom_quality_control.qc_test_question_2")
        self.cat_generic = self.env.ref(
            "custom_quality_control.qc_test_template_category_generic"
        )
