...
class AccountPurchaseWizard(models.TransientModel):
    ...
    accountAnalytic = fields.Many2one('account.analytic.account', string='Analytic account')
    ...
    def default_get(self, fields):
        ...
        for order_line in purchase.order_line:
            if order_line.account_analytic_id:
                accountAnalytic = order_line.account_analytic_id.id
            if not order_line.account_analytic_id:
        ...
        for order_line in self.purchase_id.order_line:
            ...
            if not acc:
                if order_line.product_id.name:
                    ...
                    raise ValidationError(
                    _("expense account product is empty"))
            if self.accountAnalytic:
                accountAnalytic = self.accountAnalytic.id
            else:
                accountAnalytic = False
